...
class GPSAttentionLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, alpha, attention_dropout, concat=True,thickness=1, max_dis=3, k=100, attention_size = 16, GPU = False, need_norm = True):
    ...
    def forward(self, input, receptive_field, adj):
        ...
        simple_attention_la = torch.mm(new_h, self.la_simple).view(-1)
        simple_attention_ra = torch.mm(new_h, self.ra_simple).view(-1)
        ...
        # for ii in range(self.thickness):
            ...
            #attention dropout
            recep_attention_sorted = F.dropout(recep_attention_sorted, self.attention_dropout)
...
class GPS(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout, alpha, nheads, layers,attention_hid,attention_dropout):
        ...
        self.nclass = nclass
        self.attention_size = attention_hid

        # 第一层： in: nfeat; out: nhid，即参数中的hidden-size
        self.attentions.append([
            [GPSAttentionLayer(
                nfeat, nhid, attention_size=self.attention_size,  dropout=dropout, alpha=alpha, concat=True, thickness=1, GPU=self.GPU, need_norm=True, attention_dropout=self.attention_dropout
        ...
        for i in range(layers - 2):
            self.attentions.append([
                [GPSAttentionLayer(
                    nhid * self.subheads, nhid, attention_size=self.attention_size, dropout=dropout, alpha=alpha, concat=True, thickness=i + 2, GPU=self.GPU, need_norm=True, attention_dropout=self.attention_dropout
                ) for __ in range(self.subheads)]
                for _ in range(nheads)
            ])
        ...
        self.attentions.append([
            [GPSAttentionLayer(
                nhid * self.subheads, nclass, attention_size=self.attention_size, dropout=dropout, alpha=alpha, concat=True, thickness=layers, GPU=self.GPU, need_norm=False, attention_dropout=self.attention_dropout
            ) for __ in range(self.subheads)]
            for _ in range(nheads)
        ])
